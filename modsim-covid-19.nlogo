turtles-own
  [ sick?                ;; if true, the turtle is infectious
    ...
    vaccine?             ;; if true, then the person is vaccinated
  ]
...
to setup-turtles
  create-turtles number-people
    [ setxy random-xcor random-ycor
      ...
      get-healthy ]
  ask n-of round(number-people * vaccinated-people-percentage / 100) turtles
    [ get-vaccine ]
  let i 0
  loop [
    set i i + 1
    let cx (15 - (random-float 30))
    let cy (15 - (random-float 30))
    create-turtles 10
    [ setxy (cx + (random-float 2)) (cy + (random-float 2))
      set age random lifespan
      set sick-time 0
      set remaining-immunity 0
      set vaccine? false
...
to get-sick ;; turtle procedure
  if not vaccine? or (vaccine? and vaccine-effectiveness < random-float(100))
  [
    set sick? true
    set vaccine? false
    set remaining-immunity 0
...
to update-display
  ask turtles
    [ if shape != turtle-shape [ set shape turtle-shape ]
      set color ifelse-value sick? [ red ] [ ifelse-value vaccine? [ blue ] [ ifelse-value immune? [ grey ] [ green ] ] ] ]
end
...
"healthy" 1.0 0 -10899396 true "" "plot count turtles with [ not sick? and not immune? and not vaccine? ]"
"vaccine" 1.0 0 -13345367 true "" "plot count turtles with [ vaccine? and not sick? and not immune? ]"
